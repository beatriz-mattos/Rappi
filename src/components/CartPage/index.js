...
import {
  CartProductsView, RestaurantInfos, ShippingInfo, PriceInfo
} from './styles';

import {
  MainWrapper, GenNavBar, ViewAdressCard, RadioInput, GenButton,
  ProductCard, GenText, GenHiText
      ...
      if (window.confirm('Confirmar informações do pedido?') === true) {
        ...
        for (let restaurantWithOrder in globalCart) {
          ...
          try {
            ...
            const response = await api.post(`restaurants/${restaurantWithOrder}/order`, requestBody, {
              headers: {
                auth: token
              }
            });
